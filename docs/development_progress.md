...
- Built dedicated legal pages (`src/app/terms/page.tsx`, `src/app/privacy/page.tsx`, `src/app/refund/page.tsx`) and `Footer.tsx` implementing full Paddle Merchant of Record (MoR) reseller disclosures, statutory GDPR/CCPA data rights, diagnostic telemetry details (PostHog), 14-day refund guarantee with Paddle buyer support links, support email (`vbulgariangymratt@gmail.com`), and legal cross-navigation.
